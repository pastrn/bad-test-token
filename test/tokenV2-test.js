...
//Tests for the second version of token
describe("V1 token", function() {
  ...
  async function dep() {
    ...
    const TokenFactory2 = await ethers.getContractFactory("MyToken2");
    let token = await upgrades.deployProxy(TokenFactory, [], {
      initializer: 'initialize',
      kind: 'uups',
    });
    await token.deployed();

    token = await upgrades.upgradeProxy(token.address, TokenFactory2)
